  ...
  * @version 2019.07.29
  * @Author Mukelabai Mukelabai (mukemuke2008@gmail.com)
  */
 public class FeatureSelector  implements Serializable {

     private static final long serialVersionUID = -2565931569095349011L;
     private ProjectData projectData;

     public FeatureSelector(ProjectData projectData) {
      ...
      *
      */
     public  List<FeatureSelectionResult> performSelection(String arffFilename,String xmlFilename) throws Exception {


             List<FeatureSelectionResult> featureSelectionResults = new ArrayList<>();
             ...
             String[] evaluationMethods = projectData.getConfiguration().getFeatureSelectionMethods(); //new String[]{"RF-BR", "RF-LP", "IG-BR", "IG-LP"};
             for (String evalMethod : evaluationMethods) {
                 //String  evalMethod= "IG-LP";//args[4];
                 //System.out.println("\nBuilding the feature evaluator and the problem transformation approach: " + evalMethod + "\n"); //args[4] has the name of the feature selection method to be applied

                 ASEvaluation multiLabelFeatureSelectionMethod = buildMultiLabelFeatureSelection(evalMethod, dataSet);
                 //System.out.println("\nEvaluating features by " + evalMethod + "\n");
                 Ranker r = new Ranker();
                 ...
                 for (int i = 0; i < numberFeatures; i++) {
                     double weight = sortedEvaluatedAttributeList[i][1];
                     int sttributeIndex = ((int) (sortedEvaluatedAttributeList[i][0]));
                     FeatureSelectionResult selectionResult = new FeatureSelectionResult(evalMethod.split("-")[0], evalMethod,attributes.get(sttributeIndex),weight,i+1);
                     featureSelectionResults.add(selectionResult);
...
//                 int [] featureIndices = featureIndicesByKBest(2, sortedEvaluatedAttributeList); /*Alternative function. See function documentation*/
...
//                 }
      ...
      */
     public static MultiLabelInstances buildReducedMultiLabelDataset(int[] featureIndices, MultiLabelInstances dataSet) {
