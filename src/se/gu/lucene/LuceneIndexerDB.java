...
/**
 * This classes indexes assets from the DB, unlike the other class which indexes assets that are in-memory objects
 */
public class LuceneIndexerDB {
    private ProjectData projectData;
    private SimpleAnalyzer analyzer;
    private List<String> featureNames;
    private File indexPath;
    List<AssetDB> allMappedAssets;
    private int commitIndex;
    private String currentCommit;

    public LuceneIndexerDB(ProjectData projectData, int commitIndex, String currentCommit, List<String> features, List<AssetDB> mappedAssets) throws IOException {
    ...
    public void indexAssetsForLucene() throws IOException {
        ...
        try (ProgressBar pb = new ProgressBar("Indexing assets:", size)) {
            ...
            List<AssetDB> fileAssets = allMappedAssets.stream().filter(a->a.getAssetType().equalsIgnoreCase("FILE")).collect(Collectors.toList());
            for(AssetDB file:fileAssets){
                try {
                    deleteDocumentsFromIndexUsingTerm(file.getAssetFullName());
                }catch (Exception ex){

                }
            }
            //now index the assets
            for (AssetDB asset : allMappedAssets) {
                pb.step();
    ...
    public void addAssetToIndex(AssetDB asset, IndexWriter indexWriter) throws IOException {
        ...
        document.add(new TextField("path", asset.getAssetFullName(), Field.Store.YES));
        document.add(new StringField("assetType", asset.getAssetType().toString(), Field.Store.YES));
        document.add(new TextField("parent", asset.getParent() != null ? asset.getAssetType().equalsIgnoreCase("LOC")?asset.getParent(): asset.getParent() : "", Field.Store.YES));
